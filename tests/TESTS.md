...
| `FirmCredentials` (initialization) | replaces a present but non-string host instead of letting it reach getHost() | Verifies that `host: null` (present but wrong-shaped) is replaced with the default host on a normal load and a warning is logged, since replacing a *present* invalid value discards whatever custom host was actually configured. |
| `FirmCredentials` (initialization) | stays silent when host is simply missing (legacy compatibility, not a corruption) | Verifies the missing-key default path (an older config predating this field) stays silent, unlike replacing a present invalid value. |
...
| `loadCredentials` | logs an error and falls back to {} when the credentials file parses to null instead of an object | Verifies that valid-but-wrong-shaped JSON is also treated as a load failure, closing the crash `#checkDefaultValues()` would otherwise hit via `Object.hasOwn(null, ...)`. |
| `loadCredentials` | logs an error and falls back to {} when the credentials file parses to an array instead of an object | Same as above, for a top-level array. |
| `loadCredentials` | logs an error and falls back to {} when the credentials file parses to a number instead of an object | Same as above, for a top-level number. |
...
| `field preservation across a store -> save -> load cycle` | keeps an unrecognized top-level scalar field through a load, not just per-firm fields | Verifies that a future top-level field (e.g. a schema version flag) survives a load - `#checkDefaultValues()` must treat "not an object" as "not a firm record", not as "corrupt and safe to delete". |
...
| `SilverfinAuthorizer.authorizeFirm` | should not raise errors, only debug-log, when the fetched firm name could not be saved | Verifies that a blocked `storeFirmName` save is debug-logged (matching this method's existing silent-failure convention for the firm-name path) rather than silently discarded with no signal at all. |
...
| `credentialsFileNotLoaded` | should warn that a restored backup's tokens may already be dead, not just say to restore one | Verifies the message warns that a restored backup can hold an already-rotated (dead) refresh token, not just "restore it" with no caveat. |
...
| `credentialsFileNotSaved` | should warn that a restored backup's tokens may already be dead | Same caveat as `credentialsFileNotLoaded`, for the save-refusal message. |
| `credentialsFileNotSaved` | should not claim a refresh just happened - this message also fires for --set-firm/--set-host/authorize-partner, none of which refresh anything | Verifies the wording is general ("any stored token pair"), not tied to a refresh having just occurred, since this message fires for any blocked save regardless of whether a refresh was involved. |
