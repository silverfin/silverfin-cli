...
| `FirmCredentials` (initialization) | replaces a present but non-string host instead of letting it reach getHost() | Verifies that `host: null` (present but wrong-shaped) is replaced with the default host on a normal load, the same `Object.hasOwn`-isn't-enough gap already fixed for `defaultFirmIDs`. |
