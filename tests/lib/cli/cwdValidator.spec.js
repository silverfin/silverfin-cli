  ...
  describe("run", () => {
    ...
    it("should warn about known directories", () => {
      ...
      expect(consola.warn).toHaveBeenCalledWith(
        `You are running the "silverfin" CLI from the "reconciliation_texts" directory. This could have unexpected consequences. Please, double check that you are executing "silverfin" CLI in the correct directory.`
      );
    });
    ...
    it("should display a warning if .git directory does not exist", () => {
      ...
      expect(fs.existsSync).toHaveBeenCalledWith(path.join(mockCurrentDir, ".git"));
      expect(consola.warn).toHaveBeenCalledWith('Please, double check that you are executing "silverfin" CLI in the correct directory.');
    });
  });
});
