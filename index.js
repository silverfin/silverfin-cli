...
async function publishReconciliationById(type, envId, reconciliationId, message = "Updated with the Silverfin CLI") {
  try {
    ...
    if (!matchingHandle) {
      consola.error(`No template found with reconciliation ID: ${reconciliationId}`);
      return false;
    }
    ...
    if (response && response.data && response.data.handle) {
      ...
      consola.error(`Reconciliation update failed for ID: ${reconciliationId}`);
      return false;
    }
  } catch (error) {
    errorUtils.errorHandler(error);
...
async function publishExportFileById(type, envId, exportFileId, message = "Updated with the Silverfin CLI") {
  try {
    ...
    if (!matchingHandle) {
      consola.error(`No template found with export file ID: ${exportFileId}`);
      return false;
    }
    ...
    if (response && response.data && response.data.name_nl) {
      ...
      consola.error(`Export file update failed for ID: ${exportFileId}`);
      return false;
    }
  } catch (error) {
    errorUtils.errorHandler(error);
...
async function publishAccountTemplateById(type, envId, accountTemplateId, message = "Updated with the Silverfin CLI") {
  try {
    ...
    if (!matchingHandle) {
      consola.error(`No template found with account template ID: ${accountTemplateId}`);
      return false;
    }
    ...
    if (response && response.data && response.data.name_nl) {
      ...
      consola.error(`Account template update failed for ID: ${accountTemplateId}`);
      return false;
    }
  } catch (error) {
    errorUtils.errorHandler(error);
...
async function publishSharedPartById(type, envId, sharedPartId, message = "Updated with the Silverfin CLI") {
  try {
    ...
    if (!matchingHandle) {
      consola.error(`No template found with shared part ID: ${sharedPartId}`);
      return false;
    }
    ...
    if (response && response.data && response.data.name) {
      ...
      consola.error(`Shared part update failed for ID: ${sharedPartId}`);
      return false;
    }
  } catch (error) {
    errorUtils.errorHandler(error);
...
async function addAllSharedParts(type, envId, force = false) {
  ...
  for (const sharedPartName of sharedPartsArray) {
    ...
    for (let template of sharedPartConfig.used_in) {
      ...
      if (!force) {
        const alreadyAdded = await existingLinks.find((existing) => {
...
async function getAllTemplatesId(type, envId, templateType) {
  try {
    const templates = fsUtils.getAllTemplatesOfAType(templateType);
    for (const templateName of templates) {
      const configTemplate = fsUtils.readConfig(templateType, templateName);
      const handle = configTemplate.handle || configTemplate.name || configTemplate.name_nl;
      if (!handle) {
        continue;
      }
