...
## [1.59.2] (22/09/2026)
`silverfin run-sampler` now posts the template's non-Liquid attributes alongside its Liquid: `auto_hide_formula`, `reconciliation_type`, `handle` and the localized `name_*` values. Previously only `text` and `text_parts` were sent, so a change to any of those rendered an identical before/after diff and looked like "no impact". Note this posts whatever is in the local `config.json`, not only what the branch changed — the same contract `text` already had.
