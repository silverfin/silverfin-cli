...
async function capture(url, opts = {}) {
  if (opts.handle) return captureHandle(url, opts.handle);
  return opts.full ? captureFull(url) : captureScoped(url);
}

// --handle: a targeted snapshot of a SIBLING reconciliation (its full custom —
// including line-item collections — and results), from the URL's company/period.
async function captureHandle(url, handle) {
  const target = await resolveReconciliationTarget(url, handle);
  if (target.error) {
