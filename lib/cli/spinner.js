...
class Spinner {
  ...
  static #sigintListenerRegistered = false;
  ...
  spin(text = "Doing some work...") {
    ...
    // Allow premature forced exit of the spinner (only register listener once)
    if (!Spinner.#sigintListenerRegistered) {
      const spinnerInstance = this;
      process.on("SIGINT", function () {
        spinnerInstance.running = false;
        readline.clearLine(process.stdout);
        readline.cursorTo(process.stdout, 0);
        Spinner.#addCursor();
        process.exit();
      });
      Spinner.#sigintListenerRegistered = true;
    }
