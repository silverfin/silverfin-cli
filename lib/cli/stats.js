...
// The template lists a workflow file holds, and the template type each of them is about
const WORKFLOW_TEMPLATE_TYPES = {
  reconciliations: "reconciliationText",
...
async function reportOnWorkflow(sinceDate, workflowHandle) {
  ...
  const TODAY = new Date().toJSON().toString().slice(0, 10);
  // Both the counts and the git scan must cover the same templates, so the workflow is
  // narrowed down to what the repository actually holds before either of them runs
  const workflowInRepository = narrowWorkflowToRepository(workflow);
  const templateSummary = await getWorkflowTemplateSummary(workflowInRepository);
  const yamlSummary = await getYamlSummary(sinceDate, workflowInRepository);
...
// The templates of a workflow which the repository holds, per template type
// A handle can be listed in a workflow file without ever having been imported. Counting it
// would overstate the workflow and reading its config would create the folder it is missing,
// so it is left out and named to the user
// @param {Object} workflow The workflow as it is stored
// @returns {Object} A copy of the workflow, holding only the templates present in the repository
function narrowWorkflowToRepository(workflow) {
  const templatesPresent = {};
  const missingTemplates = [];
  for (const [attribute, templateType] of Object.entries(WORKFLOW_TEMPLATE_TYPES)) {
    const listed = workflow.templates[attribute];
    const stored = fsUtils.getAllTemplatesOfAType(templateType);
    templatesPresent[attribute] = listed.filter((name) => stored.includes(name));
    missingTemplates.push(...listed.filter((name) => !stored.includes(name)));
  }
  if (missingTemplates.length > 0) {
    errorUtils.workflowTemplatesMissing(workflow.name, missingTemplates);
  }
  return { ...workflow, templates: templatesPresent };
}
