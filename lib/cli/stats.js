...
async function generateOverview(sinceDate, workflowHandle) {
  const workflow = workflowHandle ? await fsUtils.getWorkflow(workflowHandle) : null;
  const TODAY = new Date().toJSON().toString().slice(0, 10);
  const templateSummary = workflow ? await getWorkflowTemplateSummary(workflow) : await getTemplatesSummary();
  const yamlSummary = workflow ? await getYamlSummary(sinceDate, workflow) : await getYamlSummary(sinceDate);
  // Terminal
  if (workflow) {
...
async function getWorkflowTemplateSummary(workflow) {
  ...
  summary.template_name = workflow.name;
...
function displayWorkflowOverview(sinceDate, today, templateSummary, yamlSummary) {
  // Header
  consola.log("");
  consola.info(`${chalk.bold(`Workflow Summary - ${templateSummary.template_name} ( ${sinceDate} - ${today} ):`)}`);
  consola.log("------------------------------------");
  consola.log("");
  // YAML file changes
  consola.log(`New YAML files created in the period: ${yamlSummary.created}`);
