...
async function yamlFilesActivity(sinceDate) {
  ...
  if (!filesChanged) {
    console.log("No files were changed since the date provided");
    process.exit(0);
  }
...
async function generateStatsOverview(sinceDate) {
  ...
  // Modified
  console.log(`Updates to existing YAML files: ${countByType["M"] || 0}`);
  // Not showing any information about moved/renamed

  console.log("");
  console.log(`Total Reconciliations stored: ${fileCount.reconciliations}`);
  console.log(`Total YAML files stored: ${fileCount.yaml}`);
  console.log(`Total Unit Tests stored: ${fileCount.tests}`);
