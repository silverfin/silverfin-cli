...
async function watchLiquidTest(firmId, handle, testName, renderInput, templateType) {
  ...
  switch (templateType) {
    case "reconciliationText":
      ...
      filePath = path.resolve(process.cwd(), "reconciliation_texts", handle, "tests", `${handle}_liquid_test.yml`);
      break;
    case "accountTemplate":
      consola.info(`Watching for changes related to account template "${handle}" to run a new test...`);
      filePath = path.resolve(process.cwd(), "account_templates", handle, "tests", `${handle}_liquid_test.yml`);
      break;
    default:
      break;
  }
