...
// MainProcess
async function buildLiquidTest(url, testName, reconciledStatus = true) {
  ...
  return { templateHandle, liquidTestObject, templateType };
}

async function testGenerator(url, testName, reconciledStatus = true) {
  const built = await buildLiquidTest(url, testName, reconciledStatus);
  if (!built) {
    return;
  }
  // Save YAML
  Utils.exportYAML(built.templateHandle, built.liquidTestObject, built.templateType);
}
