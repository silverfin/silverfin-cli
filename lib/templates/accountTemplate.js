...
class AccountTemplate {
  ...
  static save(type, envId, template) {
    ...
    // Liquid files — write to existing config paths so subfolders are preserved on pull
    const mainPart = template.text;
    const textParts = templateUtils.filterParts(template);
    fsUtils.createTemplateFiles(this.TEMPLATE_TYPE, name, mainPart, textParts, existingConfig?.text_parts);
  ...
  static read(name) {
    ...
    this.#updateConfigIfChanged(originalConfig, templateConfig, name);
  ...
  static #prepareConfigDetails(type, envId, template, existingConfig = {}) {
    ...
    */
    ...
    const textParts = templateUtils.filterParts(template);
    const existingTextParts = existingConfig?.text_parts || {};
    const configTextParts = Object.keys(textParts).reduce((acc, name) => {
      if (name) {
        acc[name] = existingTextParts[name] || `text_parts/${name}.liquid`;
      }
      return acc;
    }, {});
  ...
  static #updateConfigIfChanged(originalConfig, currentConfig, name) {
    if (JSON.stringify(originalConfig) !== JSON.stringify(currentConfig)) {
      fsUtils.writeConfig(this.TEMPLATE_TYPE, name, currentConfig);
    }
  }
