...
class ReconciliationText {
  ...
  static read(handle, type) {
    ...
    // Certain config items are not allowed for partner 
    if (type === "partner") {
      template = this.#filterPartnerConfigItems(template);
    }
  ...
  static #filterPartnerConfigItems(filteredConfig) {
    return Object.keys(filteredConfig).reduce((acc, attribute) => {
      if (!this.PARTNER_CONFIG_ITEMS.includes(attribute)) {
        acc[attribute] = filteredConfig[attribute];
      }
      return acc;
    }, {});
  }
