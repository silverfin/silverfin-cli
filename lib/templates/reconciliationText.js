...
class ReconciliationText {
  ...
  static #prepareConfigDetails(template, existingConfig = {}) {
    ...
    // Populate missing name fields with handle if they're not set
    if (template.handle && !attributes.name_nl) {
      attributes.name_nl = template.handle;
    }
    if (template.handle && !attributes.name_en) {
      attributes.name_en = template.handle;
    }
    if (template.handle && !attributes.name_fr) {
      attributes.name_fr = template.handle;
    }
    if (template.handle && !attributes.name_de) {
      attributes.name_de = template.handle;
    }
    if (template.handle && !attributes.name_da) {
      attributes.name_da = template.handle;
    }
    if (template.handle && !attributes.name_se) {
      attributes.name_se = template.handle;
    }
    if (template.handle && !attributes.name_fi) {
      attributes.name_fi = template.handle;
    }
