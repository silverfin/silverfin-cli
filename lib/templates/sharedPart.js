...
class SharedPart {
  ...
  static async save(type, envId, template) {
    ...
    // Config Json File
    const existingConfig = fsUtils.readConfig(this.TEMPLATE_TYPE, template.name);
    const usedIn = await this.#processUsedIn(type, envId, template.used_in, existingConfig);
  ...
  static async #processUsedIn(type, envId, templateUsedIn, existingConfig) {
    ...
    for (let template of templateUsedIn) {
      ...
      const existingUsedTemplate = usedIn[templateExistingInConfig];
    ...
    if (!handle) {
      ...
      switch (template.type) {
        case "reconciliationText":
          details = await SF.readReconciliationTextById(type, envId, template.id);
          if (details) {
            handle = details.data[handle_attribute];
          }
          break;
        case "exportFile":
          details = await SF.readExportFileById(type, envId, template.id);
          if (details) {
            handle = details[handle_attribute];
          }
          break;
        case "accountTemplate":
          details = await SF.readAccountTemplateById(type, envId, template.id);
          if (details) {
            handle = details?.name_nl;
          }
          break;
      }
    }
...
//           "handle": "account_template_handle"
//       },
//       {
//           "id": 150,
