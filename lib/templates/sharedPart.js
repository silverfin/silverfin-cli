    ...
    if (!handle) {
      ...
      switch (template.type) {
        case "reconciliationText":
          details = await sfApi.reconciliationTexts.readById(type, envId, template.id);
          if (details) {
            handle = details.data[handle_attribute];
          }
          break;
        case "exportFile":
          details = await sfApi.exportFiles.readById(type, envId, template.id);
          if (details) {
            handle = details[handle_attribute];
          }
          break;
        case "accountTemplate":
          details = await sfApi.accountTemplates.readById(type, envId, template.id);
          if (details) {
