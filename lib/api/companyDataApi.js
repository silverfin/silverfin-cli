...
class CompanyDataApi extends BaseApi {
  ...
  async getPeriods(firmId, companyId, page = 1) {
    const response = await this._makeRequest("get", "firm", firmId, `/companies/${companyId}/periods`, null, { page, per_page: 200 });
  ...
  async getCompanyDrop(firmId, companyId) {
    const response = await this._makeRequest("get", "firm", firmId, `/companies/${companyId}`);
    return response.data;
  }

  async getCompanyCustom(firmId, companyId) {
    const response = await this._makeRequest("get", "firm", firmId, `/companies/${companyId}/custom`);
    return response.data;
  }

  async getWorkflows(firmId, companyId, periodId) {
    const response = await this._makeRequest("get", "firm", firmId, `/companies/${companyId}/periods/${periodId}/workflows`);
    return response.data;
  }

  async getWorkflowInformation(firmId, companyId, periodId, workflowId, page = 1) {
    const response = await this._makeRequest("get", "firm", firmId, `/companies/${companyId}/periods/${periodId}/workflows/${workflowId}/reconciliations`, null, {
      page,
      per_page: 200,
    });
    return response.data;
  ...
  async getAccountDetails(firmId, companyId, periodId, accountId) {
    const response = await this._makeRequest("get", "firm", firmId, `companies/${companyId}/periods/${periodId}/accounts/${accountId}`);
    return response.data;
  }

  async getFirmDetails(firmId) {
    const response = await this._makeRequest("get", "firm", firmId, "/user/firm");
    return response.data;
  }
}
