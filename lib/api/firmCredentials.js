...
class FirmCredentials {
  ...
  /**
   * Whether the CLI may renew this firm's credential when the platform answers 401.
   * Only an explicit boolean `false` disables it: a firm with no flag, a null, or a
   * string keeps today's behaviour, so an unseeded config is unaffected. This is the
   * mirror of how the flag is seeded in CI, which also treats anything non-boolean as
   * "no flag" - the two must agree or a firm could be seeded and ignored at once.
   * @param {Number|String} firmId
   * @returns {Boolean}
   */
  isAutoRenewEnabled(firmId) {
    return this.data?.[firmId]?.autoRenew !== false;
  }
