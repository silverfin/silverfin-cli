...
class FirmCredentials {
   ...
   * @private
   */
  /** Delete any own enumerable key of `container` whose value isn't a plain object, skipping
   * `exceptKeys`. Used to repair per-firm and per-partner records without disturbing the
   * container's own reserved keys.
   * @private
   */
  #dropMalformedEntries(container, exceptKeys = []) {
    for (const key of Object.keys(container)) {
      if (exceptKeys.includes(key)) {
        continue;
      }
      if (!this.#isPlainObject(container[key])) {
  ...
  #checkDefaultValues() {
    ...
    // hand-edited config) must be replaced, not just a missing one.
    ...
    if (Object.hasOwn(this.data, "partnerCredentials")) {
      if (!this.#isPlainObject(this.data.partnerCredentials)) {
        ...
        this.#dropMalformedEntries(this.data.partnerCredentials);
      }
    }
    if (!Object.hasOwn(this.data, "host")) {
      this.data.host = this.SF_DEFAULT_HOST;
    }
    this.#dropMalformedEntries(this.data, this.#RESERVED_DATA_KEYS);
  }
}
