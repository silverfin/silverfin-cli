...
class FirmCredentials {
   ...
   * @throws {Error} via `process.exit(1)` if the file can't be read or parsed - falling back to
   * `{}` would make the next `saveCredentials()` call overwrite the file with an empty object,
   * silently discarding every firm's tokens.
   */
  loadCredentials() {
    let credentials;
    try {
      ...
      consola.error(new Error(`Error while reading credentials file (${this.#SF_CREDENTIALS_PATH}): ${err}`));
      process.exit(1);
    }
    try {
      this.data = JSON.parse(credentials);
    } catch (err) {
      consola.error(new Error(`Credentials file is not valid JSON (${this.#SF_CREDENTIALS_PATH}): ${err}`));
      process.exit(1);
    }
  }
