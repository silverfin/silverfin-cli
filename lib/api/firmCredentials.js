...
class FirmCredentials {
  ...
  loadCredentials() {
    ...
    try {
      ...
      this.#failLoad(`Error while reading credentials file (${this.#SF_CREDENTIALS_PATH}): ${err}`);
      return;
    }
    ...
    try {
      ...
      this.#failLoad(`Credentials file is not valid JSON (${this.#SF_CREDENTIALS_PATH}): ${err}`);
      return;
    }

    if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
      this.#failLoad(`Credentials file does not contain a JSON object (${this.#SF_CREDENTIALS_PATH})`);
      return;
    }
   ...
   * Exits the process directly (`process.exit(1)`, not a catchable throw) if the last
   * `loadCredentials()` call failed - the in-memory data is an empty placeholder in that case,
   * and writing it would discard every firm's stored tokens.
   */
  saveCredentials() {
    if (this.#loadFailed) {
      consola.error(
        new Error(
          `Refusing to write ${this.#SF_CREDENTIALS_PATH}: the last load failed, so the in-memory data is incomplete. Fix or restore the credentials file, then retry.`
        )
      );
      process.exit(1);
      return;
