...
class FirmCredentials {
  ...
  storePartnerApiKey(partnerId, apiKey, partnerName = null) {
    try {
      if (!Object.hasOwn(this.data, "partnerCredentials") || typeof this.data.partnerCredentials !== "object" || this.data.partnerCredentials === null || Array.isArray(this.data.partnerCredentials)) {
   ...
   */
  #checkDefaultValues() {
    // Not just presence: setDefaultFirmId() writes into defaultFirmIDs unconditionally, so a
    // present-but-wrong-shaped value (e.g. `null` in a hand-edited config) must be replaced too.
    if (!Object.hasOwn(this.data, "defaultFirmIDs") || typeof this.data.defaultFirmIDs !== "object" || this.data.defaultFirmIDs === null || Array.isArray(this.data.defaultFirmIDs)) {
