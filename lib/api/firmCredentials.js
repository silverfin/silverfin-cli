...
class FirmCredentials {
  ...
  storeNewTokenPair(firmId, tokens) {
    // Not `|| {}`: that only replaces a falsy existing value. A truthy scalar (e.g. a hand-edited
    // "12345": "oops") throws on the assignment below under strict mode, and an array silently
    // loses the write on the next JSON.stringify - neither is a value worth preserving here, since
    // this call is specifically about populating this firm's own token record.
    if (!this.#isPlainObject(this.data[firmId])) {
      this.data[firmId] = {};
    }
  ...
  /** Return `container[key]` only if it's a plain object, treating any other value (missing,
   * `null`, or a leftover wrong-shaped scalar/array) as "no record". Every per-firm/per-partner
   * read accessor needs this same check - written once here instead of once per call site, so a
   * malformed entry can't pass one accessor's gate (e.g. `getTokenPair`) while correctly failing
   * another's (e.g. the display filter in `listAuthorizedFirms`).
   * @private
   */
  #getRecord(container, key) {
   ...
   */
  getTokenPair(firmId) {
    return this.#getRecord(this.data, firmId);
  }
  ...
  getFirmName(firmId) {
    return this.#getRecord(this.data, firmId)?.firmName || null;
  }
   ...
   */
  listAuthorizedFirms() {
    // Filtering by key name alone isn't enough: #dropNullEntries deliberately preserves an
    // unrecognized top-level field that isn't null (e.g. a future flag), so a shape check is
    // needed too, or it displays as a firm with no name.
    return Object.keys(this.data)
      .filter((element) => !this.#RESERVED_DATA_KEYS.includes(element) && this.#getRecord(this.data, element))
  ...
  getPartnerCredentials(partner_id) {
    const record = this.#getRecord(this.data.partnerCredentials, partner_id);
    if (!record) {
  ...
  listAuthorizedPartners() {
    if (Object.hasOwn(this.data, "partnerCredentials")) {
      return Object.keys(this.data.partnerCredentials)
        .filter((element) => this.#getRecord(this.data.partnerCredentials, element))
        .map((element) => ({
          id: element,
  ...
  /**
   * Get the host for the Silverfin environment: an `SF_HOST` environment variable if set,
   * otherwise the stored value (`setHost`/`#checkDefaultValues` are what apply the
   * `https://live.getsilverfin.com` default - this method doesn't).
   * Exits the process directly (`process.exit(1)`, not a catchable throw) if neither is
   * available - every caller across this codebase would otherwise build a request/URL/log line
   * from `undefined`, and there is no context in which continuing without a host is correct.
  ...
  getHost() {
    ...
    if (!this.data.host) {
      errorUtils.noHostConfigured(this.#SF_CREDENTIALS_PATH);
      process.exit(1);
    }
  ...
  #createCredentialsFile() {
    if (!fs.existsSync(this.#SF_CREDENTIALS_PATH)) {
      ...
      // If this write fails, the loadCredentials() call right after fails too (the file still
      // doesn't exist) and reports it clearly via errorUtils - this is just a debug trace for
      // -v, so the failure isn't otherwise silent at this specific step.
      if (!this.saveCredentials()) {
        consola.debug(`Could not create the initial credentials file at ${this.#SF_CREDENTIALS_PATH}`);
      }
    }
  }
  ...
  /** Repair `this.data`'s shape after a load: fill in `defaultFirmIDs`/`host` if missing (legacy
   * compatibility for a config predating one of these fields), replace them if present but
   * wrong-shaped (a hand-edited config), and drop any `null` per-firm/per-partner entry so it
   * reads as "not authorized" instead of crashing or half-succeeding elsewhere.
   * @private
   */
  #checkDefaultValues() {
    ...
    const hostPresent = Object.hasOwn(this.data, "host");
    if (!hostPresent || typeof this.data.host !== "string" || this.data.host.trim() === "") {
      // A missing key is legacy compatibility (an older config predating this field) and stays
      // silent. Replacing a *present* invalid value is different - it discards whatever
      // staging/custom host was actually configured, and the next save persists that loss, so
      // it must not happen without a word.
      if (hostPresent) {
        errorUtils.invalidHostReplaced(this.#SF_CREDENTIALS_PATH, this.data.host, this.SF_DEFAULT_HOST);
      }
