...
const { EXIT_AUTH_FAILURE } = require("../utils/authError");
  ...
  static async refreshFirm(firmId) {
    try {
      ...
      if (!firmTokens) {
        ...
        // Also the auth-failure code: a firm with no stored credential cannot be used, and no
        // amount of retrying changes that - the same question a caller is asking, same answer.
      ...
      // Same code as the interceptor's paths: a credential that cannot be renewed is a credential
      // problem wherever it is noticed. `config --refresh-token` is what the CI refresher runs, so
      // reporting it differently here would be the one place the exit code lies.
      process.exit(EXIT_AUTH_FAILURE);
    }
  }
