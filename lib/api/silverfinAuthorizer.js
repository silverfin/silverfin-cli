...
class SilverfinAuthorizer {
  ...
  static async authorizeFirm(firmId = undefined) {
    ...
    if (!this.BASE_URL) {
      // Without this, the browser opens to "undefined/f/<id>/oauth/authorize" and open()'s
      // wait:false default swallows the failure - the user is left stuck at an auth-code prompt
      // with no indication anything is wrong.
      consola.error("No host configured. Set SF_HOST or fix the credentials file, then try again");
      process.exit(1);
    }
  ...
  static async refreshFirm(firmId) {
    ...
    try {
      ...
      response = await instance.post(`${BASE_URL}/f/${firmId}/oauth/token`, data);
    } catch (error) {
      if (error.response) {
    ...
    // Outside the try/catch above: a save failure isn't a request error, and this must not be
    // caught by the same block that's already handled `process.exit`'s test-mocked throw.
    if (!firmCredentials.storeNewTokenPair(firmId, response.data)) {
      consola.error(`Refreshed tokens for firm ${firmId} could not be saved`);
      process.exit(1);
      return;
    }
  ...
  static async refreshPartner(partnerId) {
    ...
    try {
      partnerCredentials = firmCredentials.getPartnerCredentials(partnerId);
      ...
      response = await instance.post(`${BASE_URL}/api/partner/v1/refresh_api_key?api_key=${partnerCredentials.token}`);
    } catch (error) {
      if (error.response) {
    ...
    // Outside the try/catch above - see refreshFirm's comment.
    if (!firmCredentials.storePartnerApiKey(partnerId, response.data.api_key, partnerCredentials?.name)) {
      consola.error(`Refreshed API key for partner ${partnerId} could not be saved`);
      process.exit(1);
      return;
    }
  ...
  static async #getFirmAccessToken(firmId, authCode) {
    ...
    // Outside the try/catch above - see refreshFirm's comment.
    if (!firmCredentials.storeNewTokenPair(firmId, response.data)) {
      consola.error(`Tokens for firm ${firmId} could not be saved`);
      process.exit(1);
      return;
    }
  ...
  static async #getFirmName(firmId) {
    try {
      ...
      if (response && response.data) {
        if (!firmCredentials.storeFirmName(firmId, response.data.name)) {
          consola.debug(`Could not save firm name for firm ${firmId}`);
        }
      }
    } catch (error) {
