...
class SilverfinAuthorizer {
  ...
  static async authorizeFirm(firmId = undefined) {
    // getHost() itself exits if no host is available - it would otherwise open the browser to
    // "undefined/f/<id>/oauth/authorize", with open()'s wait:false default swallowing the failure.
