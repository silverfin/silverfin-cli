 ...
 * firm. The destination firm is the `envId` in the URL path; the source company and periods are passed
 * in the body. This kicks off an async job on the platform, so a successful response only means the
 * copy was accepted, not that the new company already exists.
 * @param {String} type - Environment type (only "firm" is supported)
 * @param {Number} envId - Destination firm id (where the copied company will be created)
