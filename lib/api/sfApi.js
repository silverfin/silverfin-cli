  ...
  for (const prop of properties) {
    try {
      ...
      try {
        ...
        results.push({ status: 0, statusText: networkError.message });
      }
    }
  }
  return results;
 ...
 *
 * The endpoint is firm-scoped on the v4 firm API (`/api/v4/f/:id/company_data_copier/run`): the
 * destination firm is the `:id` in the path. The firm axios instance's baseURL is already
 * `/api/v4/f/:id`, so we POST the relative `company_data_copier/run` path and inherit that baseURL
 * along with the destination firm's token, staging Basic-auth and token-refresh interceptor.
 * @param {String} type - Environment type (only "firm" is supported)
 * @param {Number} envId - Destination firm id — the `:id` in the URL path and whose token authenticates the request
