...
async function createTestRun(firmId, attributes, templateType) {
  ...
  try {
    ...
    response = await apiUtils.responseErrorHandler(error);
  }
  return response;
}
...
async function createExportFileInstance(firmId, companyId, periodId, exportFileId) {
  const instance = AxiosFactory.createInstance("firm", firmId);
  try {
    const response = await instance.post(`/companies/${companyId}/periods/${periodId}/export_file_instances`, { export_file_id: exportFileId });
    apiUtils.responseSuccessHandler(response);
    return response.data;
  } catch (error) {
    const response = await apiUtils.responseErrorHandler(error);
...
async function getExportFileInstance(firmId, companyId, periodId, exportFileInstanceId) {
  const instance = AxiosFactory.createInstance("firm", firmId);
  try {
    const response = await instance.get(`/companies/${companyId}/periods/${periodId}/export_file_instances/${exportFileInstanceId}`);
    apiUtils.responseSuccessHandler(response);
    return response.data;
  } catch (error) {
    const response = await apiUtils.responseErrorHandler(error);
...
/**
 * Trigger the Data Copier for a destination firm. It copies a source company's data (account values,
 * text properties, people/company drop and configuration) into a brand-new company in the destination
 * firm. The copy runs as a fire-and-forget async Sidekiq job: a `202 {"status":"enqueued"}` response
 * only means the copy was accepted, not that the new company already exists. There is no status endpoint
 * — the only failure signal is a destination company whose name ends in `_FAILED`.
 *
 * The endpoint lives on the non-firm-scoped public v3 API (`/api/public/v3/company_data_copier/run`):
 * the destination firm is taken from the OAuth token, NOT from the URL path. The firm axios instance's
 * baseURL forces `/api/v4/f/:id`, so we POST to an absolute public-v3 URL to bypass the baseURL while
 * still reusing the destination firm's token, staging Basic-auth and token-refresh interceptor.
 * @param {String} type - Environment type (only "firm" is supported)
 * @param {Number} envId - Destination firm id — selects which firm's token authenticates the request
 * @param {Object} attributes - { source_company_id, source_ledger_ids }
 * @returns {Object} - The axios response
 */
async function runCompanyDataCopier(type, envId, attributes) {
  const instance = AxiosFactory.createInstance(type, envId);
  const url = `${firmCredentials.getHost()}/api/public/v3/company_data_copier/run`;
  try {
    const response = await instance.post(url, attributes);
    apiUtils.responseSuccessHandler(response);
    return response;
  } catch (error) {
    const response = await apiUtils.responseErrorHandler(error);
