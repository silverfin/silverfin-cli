class SharedPartsApi {
  constructor(parentApi) {
    this.parentApi = parentApi;
  }

  async read(type, envId, page = 1) {
    const response = await this.parentApi._makeRequest("get", type, envId, "shared_parts", null, { page, per_page: 200 });
    return response.data;
  }

  async readById(type, envId, sharedPartId) {
    const response = await this.parentApi._makeRequest("get", type, envId, `shared_parts/${sharedPartId}`);
    return response.data;
  }
  ...
  async update(type, envId, sharedPartId, attributes) {
    const response = await this.parentApi._makeRequest("post", type, envId, `shared_parts/${sharedPartId}`, attributes);
    return response.data;
  }

  async create(type, envId, attributes) {
    const response = await this.parentApi._makeRequest("post", type, envId, "shared_parts", attributes);
    return response.data;
  }
}
