class AccountTemplatesApi {
  ...
  async create(type, envId, attributes) {
    const response = await this.parentApi._makeRequest("post", type, envId, "account_templates", attributes);
    return response.data;
  }

  async update(type, envId, accountTemplateId, attributes) {
    const response = await this.parentApi._makeRequest("post", type, envId, `account_templates/${accountTemplateId}`, attributes);
    return response.data;
  }

  async read(type, envId, page = 1) {
    const response = await this.parentApi._makeRequest("get", type, envId, "account_templates", null, { page, per_page: 200 });
    return response.data;
  }

  async readById(type, envId, accountTemplateId) {
    const response = await this.parentApi._makeRequest("get", type, envId, `account_templates/${accountTemplateId}`);
    return response.data;
  }
  ...
  async addSharedPart(type, envId, sharedPartId, accountTemplateId) {
    const response = await this.parentApi._makeRequest("post", type, envId, `account_templates/${accountTemplateId}/shared_parts/${sharedPartId}`);
    return response.data;
  }

  async removeSharedPart(type, envId, sharedPartId, accountTemplateId) {
    const response = await this.parentApi._makeRequest("delete", type, envId, `account_templates/${accountTemplateId}/shared_parts/${sharedPartId}`);
    return response.data;
  }
}
