...
class AxiosFactory {
  ...
  static async #refreshFirmTokens(firmId) {
    ...
    try {
      ...
      response = await axiosInstance.post(`${this.BASE_URL}/f/${firmId}/oauth/token`, data);
    } catch (error) {
      this.#handleFailedRefresh(error);
      return;
    }

    // Outside the try/catch above: a save failure isn't a request error, and retrying the
    // original request against tokens that were never actually persisted would just poison
    // the next run with the same stale pair.
  ...
  static async #refreshPartnerApiKey(partnerId) {
    ...
    try {
      ...
      response = await newAxiosInstance.post(`${this.BASE_URL}/api/partner/v1/refresh_api_key?api_key=${partnerToken}`);
    } catch (error) {
      this.#handleFailedRefresh(error);
      return;
    }

    // Outside the try/catch above - see #refreshFirmTokens's comment.
    if (!firmCredentials.storePartnerApiKey(partnerId, response.data.api_key)) {
