...
const { AuthFailureError, EXIT_AUTH_FAILURE } = require("../utils/authError");
...
class AxiosFactory {
      ...
      async (error) => {
        if (error.response) {
          if (error.response.status === 401) {
            ...
            // Checked before _retry, so a firm opted out of renewal never advances its chain.
            // Renewing here is what poisons a shared credential: the platform issues a new pair
            // and invalidates the old one, but a throwaway CI runner has nowhere to persist it,
            // so every other holder of that credential is left looking at history.
            const noRenewFirmId = originalConfig.headers["X-Firm-ID"];
            if (!firmCredentials.isAutoRenewEnabled(noRenewFirmId)) {
              consola.debug(`Auto-renew disabled for firm ${noRenewFirmId}; not refreshing its credential`);
              // Ended here rather than rejected. Rejecting looks tidier, but ~15 catch sites sit
              // between this interceptor and errorHandler and several swallow what they are given:
              // fetchAll* logs and moves to the next template, so a stale credential produced an
              // import that saved nothing and still exited 0. There is no funnel to reject into.
              // This is also what #handleFailedRefresh below already does for the same fault.
              errorUtils.errorHandler(new AuthFailureError(noRenewFirmId, error));
            }
  ...
  static #handleFailedRefresh(error) {
    ...
    // Same code as a skipped renewal: from a caller's point of view both mean "this firm's
    // credential is stale and retrying will not help". A refresh that fails is the more common
    // way to get here in CI, so the two must not report differently.
