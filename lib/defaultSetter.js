...
async function captureLight(url, handle, depth) {
  ...
  return { handle, periodOrder, currentPeriodKey: periodOrder[0] ?? null, data };
}
...
async function setDefault(url, inputPath, rawValue, opts = {}) {
  ...
  const periodEntry = light.data.periods[t.periodKey];
  if (!periodEntry) return { invertible: false, handle, input: inputPath, trace: { ...trace, invertible: false, reason: `period ${t.periodKey} not captured` } };
  const periodId = periodEntry.periodId;
