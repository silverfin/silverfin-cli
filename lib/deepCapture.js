...
// The period's account drop as { number: {number, name, account_type, value, starred} }
// (aggregate value only — no debit/credit arrays), for period.accounts / range: reads.
async function fetchPeriodAccounts(firmId, companyId, periodId) {
  const byNumber = {};
  let page = 1;
  while (page <= MAX_PAGES) {
    const response = await SF.getPeriodAccounts(firmId, companyId, periodId, page);
    const data = response?.data ?? [];
    for (const row of data) {
      const acc = row.account || {};
      const number = acc.number ?? acc.original_number;
...
async function buildDeepFixture(url, manifest, opts = {}) {
  ...
  const captureAccounts = !!opts.captureAccounts;
  ...
  for (const period of selected) {
    ...
    const entry = {
      periodId,
      // Period DROP scalars (from getPeriods) so the offline evaluator can read
      // period.year_end_date / year_start_date / end_date / fiscal_year.*.
      drop: {
        year_end_date: key,
        year_start_date: period.fiscal_year?.start_date ?? null,
        end_date: period.end_date ?? null,
        fiscal_year: period.fiscal_year ?? null,
      },
      custom: Utils.processCustom((await SF.getAllPeriodCustom(firmId, companyId, periodId)) || []),
      reconciliations: {},
    };
    ...
    if (captureAccounts) {
      entry.accounts = await fetchPeriodAccounts(firmId, companyId, periodId);
    }
