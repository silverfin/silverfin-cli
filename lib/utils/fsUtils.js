...
function createTemplateFiles(templateType, handle, textMain, textParts, configTextParts = {}) {
  try {
    // Template: Main
    const mainPath = path.join(process.cwd(), `${FOLDERS[templateType]}`, `${handle}`, "main.liquid");
    fs.writeFileSync(mainPath, textMain);
    // Template: Parts — use config path if present (supports subfolders), otherwise flat
    const templateDir = path.join(process.cwd(), FOLDERS[templateType], handle);
    Object.keys(textParts).forEach((textPartName) => {
      if (!textPartName) return;
      const relativePart = configTextParts[textPartName] || `text_parts/${textPartName}.liquid`;
      const partPath = path.join(templateDir, relativePart);
...
// @returns {Array<string>} Array with the handles of every workflow stored in the workflows folder
// @returns {Array<string>} An empty array is returned when the folder is missing or holds no workflow files
function getAllWorkflowHandles() {
  const workflowsPath = path.join(process.cwd(), WORKFLOWS_FOLDER);
...
// Read a workflow from the workflows folder
// Returns undefined when the handle is unsafe, or when the workflow cannot be found, parsed
// or misses required attributes. It is up to the caller to decide how severe that is
function getWorkflow(workflowHandle) {
  // The handle is the name of a file in the workflows folder. It cannot be trusted to stay
  // inside it, and this function must not exit the process, so an unsafe handle is refused here
  if (!templateUtils.isSafeName(workflowHandle)) {
    errorUtils.invalidWorkflowHandle(workflowHandle);
    return undefined;
  }

  const workflowPath = path.join(process.cwd(), WORKFLOWS_FOLDER, `${workflowHandle}.json`);

  if (!fs.existsSync(workflowPath)) {
    errorUtils.missingWorkflow(workflowHandle, getAllWorkflowHandles());
    return undefined;
  }
  ...
  try {
    ...
    consola.debug(error);
    errorUtils.unparsableWorkflow(workflowHandle, error.message);
    return undefined;
  }

  const problems = workflowAttributeProblems(workflow);
  if (problems.length > 0) {
    errorUtils.invalidWorkflow(workflowHandle, problems);
    return undefined;
  }
...
// Check that a workflow holds every attribute the CLI relies on
// @param {Object} workflow The workflow to check
// @returns {Array<string>} An array of messages, empty when the workflow is valid
function workflowAttributeProblems(workflow) {
  const problems = [];
  if (!workflow || typeof workflow !== "object" || Array.isArray(workflow)) {
    return [`it should contain a JSON object`];
  }
  if (!workflow.name || typeof workflow.name !== "string") {
    problems.push(`"name" is missing or is not a text value`);
  }
  if (!workflow.templates || typeof workflow.templates !== "object" || Array.isArray(workflow.templates)) {
    problems.push(`"templates" is missing or is not an object`);
    return problems;
  }
  for (const attribute of ["reconciliations", "accounts", "exports"]) {
    if (!Array.isArray(workflow.templates[attribute])) {
      problems.push(`"templates.${attribute}" is missing or is not an array`);
    }
  }
  return problems;
}
...
/**
 * Recursively scan a text_parts directory and build a map of part names to their relative paths.
 * This supports nested subdirectories inside text_parts/.
 * @param {string} templateType Options: `reconciliationText`, `accountTemplate`, `exportFile`
 * @param {string} handle Handle or name of the template
 * @returns {Object} Map of { partName: "text_parts/[subdir/]partName.liquid" }
 */
  ...
  function scanDirectory(dirPath, relativeTo) {
    ...
    for (const entry of entries) {
      ...
      if (stats.isDirectory()) {
        ...
        const partName = path.basename(entry, ".liquid");
        const relativePath = path.relative(relativeTo, fullPath);
        const configPath = relativePath;

        if (partsMap[partName]) {
          duplicates.push({ name: partName, path1: partsMap[partName], path2: configPath });
        }

        partsMap[partName] = configPath;
      }
    }
  }

  const templateDir = path.join(process.cwd(), FOLDERS[templateType], handle);
  scanDirectory(textPartsDir, templateDir);

  if (duplicates.length > 0) {
    for (const dup of duplicates) {
      consola.warn(`Duplicate text part name "${dup.name}" found at "${dup.path1}" and "${dup.path2}". Using "${dup.path2}".`);
    }
  }
