...
function lookForInputTags(liquidCode, input_type = "") {
  ...
  let reInput;
  if (input_type) {
    const expression = `{%.*input.*as:${input_type}.*%}`; // {% input as:account_collection %}
...
function lookForDefault(text) {
  ...
  if (Array.isArray(text)) {
    const defaultsArray = text
      .map((element) => {
        const matchItem = element.match(reDefault);
        if (matchItem) {
...
function lookForAssign(text, variableName) {
  const expression = `{%.*assign.*${variableName}.*=.*%}`; // {% assign variable_name = content %}
