...
function setAxiosStagingEnv(firmId) {
  let stagingRegex = new RegExp(/\.staging\./);
  let stagingEnv = stagingRegex.test(BASE_URL);
  if (stagingEnv && process.env.SF_BASIC_AUTH) {
    const firmTokens = firmCredentials.getTokenPair(firmId);
    console.log("...Staging environment...");
    axios.defaults.headers.common[
      "Authorization"
    ] = `Basic ${process.env.SF_BASIC_AUTH}`;
    axios.defaults.params = { access_token: firmTokens.accessToken };
  }
}
