...
function credentialsFileNotLoaded(path, reason) {
  ...
  consola.log(`The CLI will continue, but cannot save new tokens until this is fixed. Fix or restore ${path} and run the command again`);
  return false;
}
...
function credentialsFileNotSaved(path) {
  ...
  consola.log(`Fix or restore ${path} and run the command again`);
  return false;
}
 ...
 * @param {string} path
 * @param {Error} error Kept for the debug output of the caller
 * @returns {boolean} False
 */
