...
function credentialsFileNotSaved(path) {
  ...
  consola.log(
    `Fix ${path} and run the command again - if you restore it from a backup instead, re-authorize afterward: the refresh this run just performed may have already rotated the server-side token, making a restored pair dead on arrival`
  );
  return false;
}
