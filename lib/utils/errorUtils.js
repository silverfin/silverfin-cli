...
/**
 * getHost() has no usable host: no `SF_HOST` override and the credentials file has none stored
 * (typically because the last load failed - see #failLoad, which deliberately doesn't default
 * host to avoid silently redirecting a staging user's session to production). Every caller of
 * getHost() would build a request/URL/log line from `undefined` if this weren't caught here.
 * @param {string} path
 */
function noHostConfigured(path) {
  consola.error(`No host is configured for the Silverfin API`);
  consola.log(`Set SF_HOST, or fix ${path} and run the command again`);
}

/**
 * #checkDefaultValues() replaced a present but invalid `host` value with the default. Unlike a
 * missing key (legacy compatibility, silent), replacing a *present* value - and then persisting
 * it on the next save - discards whatever custom host was actually configured, so it must not
 * happen without a word.
 * @param {string} path
 * @param {*} invalidValue The value that was replaced - truncated if unreasonably long, since it
 * comes directly from the credentials file's own content
 * @param {string} defaultHost
 */
function invalidHostReplaced(path, invalidValue, defaultHost) {
  const shown = JSON.stringify(invalidValue);
  const truncated = shown.length > 100 ? `${shown.slice(0, 100)}...` : shown;
  consola.warn(`Credentials file ${path} had an invalid host (${truncated}); resetting it to the default (${defaultHost})`);
}
