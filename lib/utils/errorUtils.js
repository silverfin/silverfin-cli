...
/**
 * An id provided on the command line is not a positive integer. Every Silverfin id is one, so the
 * value is a typo or an unset variable rather than something the platform could have answered for
 * @param {string|number} value The id provided by the user
 * @param {string} label How to name the id in the message (e.g. "firm id")
 * @returns {boolean} False
 */
function invalidNumericId(value, label) {
  consola.error(`Invalid ${label} "${value}". A ${label} is a number, for example 13827`);

  // A padded id is the one rejected value which is not a typo: it names a real id, and the CLI
  // stores tokens under the exact string it was given, so a firm authorized as "7" cannot be
  // reached as "007". Naming the unpadded id saves the user guessing why they are refused
  const padded = /^0+([1-9]\d*)$/.exec(String(value));
  if (padded) {
    consola.log(`Did you mean ${padded[1]}?`);
    return false;
  }

  consola.log(`Please check the value you passed, or check that the variable you passed is set`);
  return false;
}

/**
 * A date provided on the command line is not written as YYYY-MM-DD
 * @param {string} dateString The date provided by the user
 * @returns {boolean} False
 */
function invalidDateFormat(dateString) {
  consola.error(`Invalid date "${dateString}". Please provide a date using the format YYYY-MM-DD (e.g. 2024-01-31)`);
  return false;
}

/**
 * A date provided on the command line is written as YYYY-MM-DD but is not a day which exists
 * @param {string} dateString The date provided by the user
 * @returns {boolean} False
 */
function impossibleDate(dateString) {
  consola.error(`Invalid date "${dateString}". This is not an existing calendar date`);
  return false;
}
