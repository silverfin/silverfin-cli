...
function checkValidName(name, templateType) {
  ...
  if (!nameCheck) {
    consola.warn(
      `Template name "${name}" contains invalid characters. Skipping. ${templateType === "accountTemplate" || templateType === "exportFile"
...
function missingLiquidCode(template) {
  if (!template?.text) {
    consola.warn(
      `Template "${template?.handle || template?.name || template?.name_nl
...
function missingNameNL(template) {
  if (!template?.name_nl) {
    consola.warn(
      `Template name_nl is missing "${template?.name_en ||
      template?.name_fr ||
      template?.name_da ||
      template?.name_de
