...
function unclosedTagCount(html) {
  ...
  while (i !== -1) {
    ...
    // Find the tag's `>`, skipping quoted values. An unterminated quote or a
    // missing `>` ends the scan: nothing after it can be a tag either.
    while (j < html.length && html[j] !== ">") {
      if (html[j] === '"' || html[j] === "'") {
        const quoteEnd = html.indexOf(html[j], j + 1);
        if (quoteEnd === -1) return countOpen(open);
        j = quoteEnd + 1;
      } else {
        j += 1;
      }
