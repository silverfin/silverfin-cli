...
// this the diff stops being useful at all.
const CAP_TIERS = [
  { changes: Infinity, setItems: Infinity, visualChanges: Infinity, visualGroups: Infinity, visualEntries: Infinity, tagDeltas: Infinity },
  { changes: 60, setItems: 25, visualChanges: 50, visualGroups: 40, visualEntries: 30, tagDeltas: 25 },
  { changes: 25, setItems: 10, visualChanges: 20, visualGroups: 20, visualEntries: 12, tagDeltas: 12 },
  {
    changes: MAX_CHANGES_SHOWN,
    setItems: MAX_SET_ITEMS_SHOWN,
    visualChanges: MAX_VISUAL_CHANGES_SHOWN,
    visualGroups: MAX_VISUAL_GROUPS_SHOWN,
    visualEntries: MAX_VISUAL_ENTRIES_LISTED,
    tagDeltas: MAX_TAG_DELTAS_SHOWN,
  },
];
...
function describeFlagFlipSuffix(before, after) {
  ...
  if (flips.length <= activeCaps.setItems) return ` (${flips.join(", ")})`;
  const hash = crypto.createHash("sha1").update(flips.join(",")).digest("hex").slice(0, 8);
  return ` (${flips.slice(0, activeCaps.setItems).join(", ")} +${flips.length - activeCaps.setItems} more, #${hash})`;
}
 ...
 */
function previewList(items, max = activeCaps.setItems) {
...
function elementNotes(before, after) {
  ...
  if (deltas.length) {
    ...
    return [`structure: ${previewList(deltas.map((d) => `${codeSpan(`<${d.tag}>`)} ${d.b} → ${d.a}`), activeCaps.tagDeltas)}`];
  }
