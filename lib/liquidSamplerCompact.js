...
/**
 * @param {string} html
 * @returns {number} opening non-void tags minus closing tags - a linear
 *   pre-check, since measuring this by parsing is the cost being avoided
 */
function unclosedTagCount(html) {
  let open = 0;
  for (const match of html.matchAll(/<([a-zA-Z][\w:-]*)[^>]*?(\/?)>/g)) {
    if (!match[2] && !VOID_TAGS.has(match[1].toLowerCase())) open += 1;
  }
  const closed = (html.match(/<\/[a-zA-Z]/g) || []).length;
  return open - closed;
}
