...
function previewSet(items, max = MAX_SET_ITEMS_SHOWN) {
  // One line per item: a handle or ledger id from the zip may contain a line
  // break, and the next line could then open a code fence.
  const shown = items.map(oneLine);
  if (shown.length <= max) return shown.join(", ");
  const hash = crypto.createHash("sha1").update(items.join("\n")).digest("hex").slice(0, 8);
  return `${previewList(shown, max)}, #${hash}`;
}
...
const ENTRY_ID_ATTRS = /(\s(?:data-object-id|data-object-ledger-id))\s*=\s*(?:"[^"]*"|'[^']*'|[^\s>]+)/gi;
// Only applied inside a tag, so visible text that happens to read like the
// attribute is never normalized away.
// Quoted values may contain `<`/`>` (measured linear on unterminated quotes).
...
function normalizeEntryIds(html) {
  return html.replace(OPENING_TAG, (tag) => tag.replace(ENTRY_ID_ATTRS, '$1=""'));
}
 ...
 *   tag of one kind can't cancel an unclosed one of another. A single forward
 *   scan rather than a regex: quoted attribute values may contain `<`/`>`, and
 *   every regex that allows that rescans the rest of the input on an
 *   unterminated quote.
 */
function unclosedTagCount(html) {
  const open = new Map();
  // Quote characters with no later closer. Once one is found there is none
  // after any later position either, so it is never searched for again.
  ...
  while (i !== -1) {
    ...
    j = TAG_NAME.lastIndex;
    // Find the tag's `>`, skipping quoted values. A missing `>` ends the scan.
    let afterEquals = false;
    while (j < html.length && html[j] !== ">") {
      const c = html[j];
      // As in HTML, a quote only opens a value right after `=`; anywhere else
      // (or with no closer) it is an ordinary character, and the scan goes on.
      if (afterEquals && (c === '"' || c === "'") && !unmatched.has(c)) {
        const quoteEnd = html.indexOf(c, j + 1);
        if (quoteEnd !== -1) {
          j = quoteEnd + 1;
          afterEquals = false;
          continue;
    ...
    if (!selfClosing && !VOID_TAGS.has(tag) && !IMPLICITLY_CLOSED_TAGS.has(tag)) {
  ...
  return countOpen(open);
}

/**
 * @param {Map<string, number>} open
 * @returns {number}
 */
function countOpen(open) {
  let total = 0;
...
function formatChangeLines(changes) {
  ...
  for (const change of changes.slice(0, activeCaps.changes)) {
    ...
    const body = change.summary !== undefined ? change.summary : `${codeSpan(change.before)} → ${codeSpan(change.after)}`;
