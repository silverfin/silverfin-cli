...
// Closed implicitly by the parser, so leaving them open costs nothing.
const IMPLICITLY_CLOSED_TAGS = new Set(["li", "p", "td", "th", "tr", "option", "dt", "dd", "thead", "tbody", "tfoot"]);
...
function unclosedTagCount(html) {
  ...
  for (const [, closing, name, selfClosing] of html.matchAll(/<(\/?)([a-zA-Z][\w:-]*)[^<>]*?(\/?)>/g)) {
    ...
    open.set(tag, (open.get(tag) || 0) + (closing ? -1 : 1));
  }
  let total = 0;
  for (const count of open.values()) total += Math.max(0, count);
  return total;
}
