...
function describeOptionChange(name, before, after) {
  ...
  const parts = [];
  if (diff && diff.removed.length) parts.push(`lost: ${previewList(diff.removed.map(codeSpan))}`);
  if (diff && diff.added.length) parts.push(`added: ${previewList(diff.added.map(codeSpan))}`);
...
function describeFieldChanges(before, after) {
  ...
  for (const name of [...new Set([...before.keys(), ...after.keys()])].sort()) {
    ...
    if (b.tag !== a.tag) {
      notes.push(`field \`${name}\` element: \`<${b.tag}>\` → \`<${a.tag}>\``);
    }
    if (b.value !== a.value) {
...
function elementNotes(before, after) {
  ...
  if (deltas.length) {
    ...
    return [`structure: ${previewList(deltas.map((d) => `\`<${d.tag}>\` ${d.b} → ${d.a}`), MAX_TAG_DELTAS_SHOWN)}`];
  }
...
function tableNotes(before, after) {
  ...
  // every later pair a comparison of two different tables. Report the count
  // (elementNotes is skipped on the field-note path) and compare no further.
  if (before.tables.length !== after.tables.length) {
    return [`tables: ${before.tables.length} → ${after.tables.length}`];
  }
  const notes = [];
...
function describeViewHtmlDiff(resultsDir, kind, entryId) {
  ...
  let afterHtml;
  try {
    if (fs.statSync(beforePath).size > MAX_VIEW_HTML_BYTES || fs.statSync(afterPath).size > MAX_VIEW_HTML_BYTES) {
