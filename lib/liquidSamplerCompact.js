...
function formatCompact(data) {
  ...
  if (visualOnlyEntries.length > 0) {
    ...
    for (const group of groups.slice(0, MAX_VISUAL_GROUPS_SHOWN)) {
      ...
      lines.push(`**${group.label}** — ${shared}${ref}\`output/${entryKey}/{before,after}/view.html\``);
      ...
      if (group.entries.length > 1) {
        lines.push(`- entries: ${previewList(group.entries.map((entry) => entry.entryId), MAX_VISUAL_ENTRIES_LISTED)}`);
      }
    }
