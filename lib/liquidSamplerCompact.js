...
// reallocation look like a layout change.
const ENTRY_ID_ATTRS = /\s(?:data-object-id|data-object-ledger-id)="[^"]*"/g;

/**
 * @param {string} html
 * @returns {string} the same markup with per-entry object ids stripped
 */
function normalizeEntryIds(html) {
  return html.replace(ENTRY_ID_ATTRS, "");
}
...
function extractNamedFields(root) {
  ...
  for (const el of root.querySelectorAll("[data-name]")) {
    ...
    if (tag === "textarea") {
      fields.set(name, { tag, value: el.textContent.trim(), placeholder: el.getAttribute("placeholder") ?? null, options: null });
      continue;
    }
    if (tag === "select") {
      ...
      fields.set(name, {
        tag,
        value: selected ? selected.textContent.trim() : null,
        placeholder: null,
    ...
    if ((el.getAttribute("type") || "").toLowerCase() === "radio") {
      const group = fields.get(name) || { tag, value: null, placeholder: null, options: [] };
...
function describeOptionChange(name, before, after) {
  ...
  const parts = [];
  if (diff && diff.removed.length) parts.push(`lost: ${previewList(diff.removed)}`);
  if (diff && diff.added.length) parts.push(`added: ${previewList(diff.added)}`);
  const detail = parts.length ? ` (${parts.join("; ")})` : "";
  return `field \`${name}\` options: ${before.length} → ${after.length}${detail}`;
}
...
function elementNotes(before, after) {
  ...
  if (deltas.length) {
    ...
    return [`structure: ${previewList(deltas.map((d) => `\`<${d.tag}>\` ${d.b} → ${d.a}`))}`];
  }
 ...
 * @returns {Array<string>}
 */
function tableNotes(before, after) {
  const notes = [];
  for (let i = 0; i < Math.min(before.tables.length, after.tables.length); i += 1) {
    ...
    if (widths.length) notes.push(`table ${i + 1} column span: ${previewList(widths)}`);
  }
  return notes;
}
...
function describeTextChange(before, after) {
  ...
  const diff = diffStringSet(words(before.text), words(after.text));
  if (!diff) return "static text reordered (same words)";
  return `static text: ${formatSetSummary(diff.added, diff.removed, "word")}`;
}
...
function describeVisualChange(beforeHtml, afterHtml) {
  ...
  if (fieldNotes.length) return [...fieldNotes, ...tableNotes(before, after)];
...
function describeViewHtmlDiff(resultsDir, kind, entryId) {
  ...
  const beforeHtml = normalizeEntryIds(fs.readFileSync(beforePath, "utf8"));
  const afterHtml = normalizeEntryIds(fs.readFileSync(afterPath, "utf8"));
...
/**
 * Wrap text in a Markdown code span. Backticks are dropped rather than
 * escaped: with `--from-zip` an entry id is a directory name from an
 * arbitrary local zip, and one stray backtick closes the span early and lets
 * the rest of the id render as Markdown in the posted comment.
 * @param {string} text
 * @returns {string}
 */
...
function formatCompact(data) {
  ...
  if (visualOnlyEntries.length > 0) {
    const groups = groupVisualOnlyEntries(visualOnlyEntries);
    lines.push("");
    lines.push(
    ...
    for (const group of groups.slice(0, MAX_VISUAL_GROUPS_SHOWN)) {
      lines.push("");
      ...
      lines.push(`**${group.label}** — ${shared}${ref}${codeSpan(`output/${entryKey}/{before,after}/view.html`)}`);
      for (const note of group.changes.slice(0, MAX_VISUAL_CHANGES_SHOWN)) {
        lines.push(`- ${note}`);
      }
      if (group.changes.length > MAX_VISUAL_CHANGES_SHOWN) {
        const hidden = group.changes.length - MAX_VISUAL_CHANGES_SHOWN;
        lines.push(`- … +${hidden} more change${hidden === 1 ? "" : "s"}`);
      }
      if (group.entries.length > 1) {
        lines.push(`- entries: ${previewList(group.entries.map((entry) => codeSpan(entry.entryId)), MAX_VISUAL_ENTRIES_LISTED)}`);
      }
    }
    if (groups.length > MAX_VISUAL_GROUPS_SHOWN) {
      const hiddenGroups = groups.length - MAX_VISUAL_GROUPS_SHOWN;
      const hiddenEntries = groups.slice(MAX_VISUAL_GROUPS_SHOWN).reduce((sum, group) => sum + group.entries.length, 0);
      lines.push("");
