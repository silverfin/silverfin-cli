...
// A fiscal_year value that stringifies to the year number (templates index
// custom maps with `[period.fiscal_year]`) while still exposing .start_date/.end_date.
function makeFiscalYear(fy, yearEndDate) {
  const src = fy && typeof fy === "object" ? fy : {};
  const ym = String(src.end_date || yearEndDate || "").match(/^(\d{4})/);
  const year = ym ? ym[1] : "";
  return {
    start_date: src.start_date || null,
    end_date: src.end_date || yearEndDate || null,
    toString: () => year,
    valueOf: () => (year ? Number(year) : NaN),
  };
}

// period drop (with minus_Ny + accounts), reconciliations, company, current_reconciliation.
function buildContext(deep) {
  const byOffset = (deep.periodOrder || []).map((key) => {
    ...
    const drop = p.drop || {};
    const period = {
      year_end_date: drop.year_end_date || key,
      year_start_date: drop.year_start_date || null,
      end_date: drop.end_date || null,
      fiscal_year: makeFiscalYear(drop.fiscal_year, drop.year_end_date || key),
      exists: true,
      reconciliations: recon,
      custom: p.custom || {},
    };
    if (p.accounts) {
      const arr = Object.values(p.accounts);
      period.accounts = arr;
      period.year_end = { accounts: arr }; // period.minus_Ny.year_end.accounts alias
    }
    return period;
  });
  ...
  const companyData = (deep.data && deep.data.company) || {};
  const companyDrop = companyData.drop && typeof companyData.drop === "object" ? companyData.drop : {};
  const company = { custom: companyData.custom || {}, drop: companyDrop };
  // Flatten the drop's top-level scalars onto company.* (name, vat_identifier, ...).
  for (const [k, v] of Object.entries(companyDrop)) {
    if (v === null || typeof v !== "object") company[k] = v;
  }

  return { period, company, current_reconciliation: { handle: deep.handle } };
}
...
async function computeOfflineDefaults(url, scope, rows) {
  ...
  // Only pay for the (heavier) account capture when the template actually reads accounts.
  const usesAccounts = /period(?:\.minus_\d+y)?\.accounts|\byear_end\.accounts|\brange:/.test(liquid);
  const deep = await buildDeepFixture(url, scope, {
    maxPriorPeriods: scope.priorPeriodDepth || 0,
    captureAccounts: usesAccounts,
  });
  if (!deep) {
