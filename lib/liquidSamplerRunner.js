...
const SAMPLER_ATTRS_BY_TYPE = {
  reconciliation_text: ["text", "text_parts", "auto_hide_formula", "reconciliation_type"],
  account_detail_template: ["text", "text_parts"],
  shared_part: ["text"],
};

/**
 * Project a locally-read template onto the attributes the sampler API accepts.
 * Absent keys are omitted rather than sent as undefined - the backend assigns
 * whatever arrives, so a stray undefined would null out the partner's value.
...
function pickSamplerAttrs(templateContent, type) {
  ...
  for (const key of SAMPLER_ATTRS_BY_TYPE[type]) {
    if (templateContent[key] !== undefined) attrs[key] = templateContent[key];
  }
  return attrs;
}
