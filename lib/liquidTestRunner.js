...
async function getHTML(url, testName, openBrowser = false, mode) {
  const filePath = resolveHTMLPath(`${testName}_${mode}`);
...
async function handleHTMLfiles(testName = "", testRun, mode) {
  deleteExistingHTMLs();

  const getHTMLrenders = async (mode, testName, testRun, openBrowser) => {
    switch (mode) {
      case "all":
        await getHTML(
          testRun.tests[testName].html_input,
          testName,
          openBrowser,
          "html_input"
        );
        await getHTML(
          testRun.tests[testName].html_preview,
          testName,
          openBrowser,
          "html_preview"
        );
  ...
  if (testName) {
    // Only one test
    getHTMLrenders(mode, testName, testRun, true);
  } else {
    // All tests
...
async function runTestsWithOutput(
  ...
  try {
    ...
    if (
      ...
      handleHTMLfiles(testName, testsRun.previewRun, mode);
    }
  } catch (error) {
    errorUtils.errorHandler(error);
  }
