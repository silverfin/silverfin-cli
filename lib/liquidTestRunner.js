...
async function runTests(firmId, handle, testName = "", mode) {
  try {
...
async function runTestsWithOutput(
  ...
  try {
    ...
    const mode = runTestUtils.checkMode(htmlInput, htmlPreview);
    const testRun = await runTests(firmId, handle, testName, mode);
    if (!testRun) return;
