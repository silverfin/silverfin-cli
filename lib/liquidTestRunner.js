...
function filterTestsByBatch(testContent, batchIdentifier, testIndexes) {
  const indexes = testIndexes || findTestRows(testContent);
  const matchingTests = Object.keys(indexes).filter((testName) => testName.includes(batchIdentifier));
...
function buildTestParams(firmId, templateType, handle, testName = "", renderMode, batch = "") {
  ...
  if (batch) {
    const { filteredContent, matchingTests, lineAdjustments: batchLineAdjustments } = filterTestsByBatch(testContent, batch, testIndexes);

    if (!matchingTests.length) {
      consola.error(`No tests found containing "${batch}" in their name`);
      process.exit(1);
    }

    finalTests = filteredContent;
    lineAdjustments = batchLineAdjustments;
    consola.info(
      `Running ${matchingTests.length} test${matchingTests.length === 1 ? "" : "s"} containing "${batch}": ${matchingTests.join(", ")}`
    );
  }
...
async function runTests(firmId, templateType, handle, testName = "", previewOnly = false, renderMode = "none", batch = "") {
  try {
    ...
    const buildResult = buildTestParams(firmId, templateType, handle, testName, renderMode, batch);
