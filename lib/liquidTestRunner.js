...
async function runTestsWithOutput(firmId, templateType, handle, testName = "", previewOnly = false, htmlInput = false, htmlPreview = false, batched = false) {
  try {
    if (templateType !== "reconciliationText" && templateType !== "accountTemplate") {
      consola.error(`Template type is missing or invalid`);
      process.exit(1);
    ...
    if (batched && !testName) {
      ...
      // Run all tests concurrently: batch size equals total number of tests
      const batches = chunkArray(allTestNames, allTestNames.length);
