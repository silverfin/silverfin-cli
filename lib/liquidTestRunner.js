...
function getAllTestNames(templateType, handle) {
  ...
  switch (testRun.status) {
    ...
    case "completed":
      ...
      if (errorsPresent === false) {
        ...
        const failedTests = tests.filter((testName) => checkTestErrorsPresent(testName, testRun.tests));
        consola.log(chalk.red(`${failedTests.length} TEST${failedTests.length !== 1 ? "S" : ""} FAILED`));
...
async function runTestsWithOutput(firmId, templateType, handle, testName = "", previewOnly = false, htmlInput = false, htmlPreview = false, batched = false, batchSize = null) {
  try {
    if (templateType !== "reconciliationText" && templateType !== "accountTemplate") {
      consola.error(`Template type is missing or invalid`);
      process.exit(1);
    ...
    // Batched mode: split all tests and run per-test concurrently in batches
    if (batched && !testName) {
      const allTestNames = getAllTestNames(templateType, handle);
      if (allTestNames.length === 0) {
        consola.info(`${handle}: there are no tests stored in the YAML file`);
        return;
      }

      // Default to all tests at once, but allow limiting batch size if provided
      const effectiveBatchSize = batchSize ? Math.max(1, Number(batchSize) || allTestNames.length) : allTestNames.length;
      const batches = chunkArray(allTestNames, effectiveBatchSize);
      const aggregated = { status: "completed", tests: {} };
      const erroredRuns = [];

      for (const group of batches) {
        const results = await Promise.all(
          group.map(async (singleTestName) => {
            try {
              const testsRun = await runTests(firmId, templateType, handle, singleTestName, previewOnly, renderMode);
              return { name: singleTestName, result: testsRun };
            } catch (error) {
              return { name: singleTestName, result: null, error };
            }
          })
        );
        ...
        for (const { name, result, error } of results) {
          if (error || !result) {
            erroredRuns.push({ name, error: error || new Error("Unknown error") });
            continue;
          }

          const testRun = result?.testRun;
          const previewRun = result?.previewRun;

          if (testRun && testRun.status === "completed" && testRun.tests) {
            Object.assign(aggregated.tests, testRun.tests);
          } else if (testRun && (testRun.status === "test_error" || testRun.status === "internal_error")) {
            erroredRuns.push({ name, error: new Error(testRun.error_message || testRun.status) });
          }

          if (previewRun && previewRun.status !== "test_error" && renderMode !== "none") {
            await getHTMLrenders(renderMode, name, previewRun, true);
          }
        }
      }

      // If any errored, inform and set status
      if (erroredRuns.length > 0) {
        aggregated.status = "completed"; // Keep completed so we can still print aggregated
        consola.log("");
        for (const error of erroredRuns) {
          consola.error(`Test "${error.name}" failed to run: ${error.error?.message || "Unknown error"}`);
        }
      }

      processTestRunResponse(aggregated, previewOnly);
      return;
    }
...
// Shows minimal output: handle name + status, and list of failed test names if any
async function runTestsStatusOnly(firmId, templateType, handle, testName = "", batched = false, batchSize = null) {
  if (templateType !== "reconciliationText" && templateType !== "accountTemplate") {
    consola.error(`Template type is missing or invalid`);
    process.exit(1);
  }

  // Batched mode: run per-test concurrently
  if (batched && !testName) {
    const allTestNames = getAllTestNames(templateType, handle);
    if (allTestNames.length === 0) {
      consola.info(`${handle}: there are no tests stored in the YAML file`);
    ...
    for (const group of batches) {
      const results = await Promise.all(
        group.map(async (singleTestName) => {
          try {
            const testsRun = await runTests(firmId, templateType, handle, singleTestName, false, "none");
            return { name: singleTestName, result: testsRun };
          } catch (error) {
            return { name: singleTestName, result: null, error };
          }
    ...
    // Check results
    if (erroredRuns.length > 0) {
      consola.log(`${handle}: FAILED`);
      erroredRuns.forEach((er) => {
        consola.log(`  ${er.name}: FAILED`);
      });
      process.exit(1);
      return "FAILED";
    }

    const errorsPresent = checkAllTestsErrorsPresent(aggregated.tests);
    if (errorsPresent === false) {
      consola.log(`${handle}: PASSED`);
      return "PASSED";
    } else {
      const tests = Object.keys(aggregated.tests).sort();
      const failedTests = tests.filter((testName) => checkTestErrorsPresent(testName, aggregated.tests));
      consola.log(`${handle}: FAILED`);
