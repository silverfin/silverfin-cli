...
async function fetchResult(firmId, testRunId, templateType) {
  ...
  while (testRun.status === "started") {
    ...
    const response = await sfApi.testing.readTestRun(firmId, testRunId, templateType);
...
async function runTests(firmId, templateType, handle, testName = "", previewOnly = false, renderMode = "none") {
  try {
    ...
    if (renderMode !== "none") {
      const previewRunResponse = await sfApi.testing.createPreviewRun(firmId, testParams, templateType);
      const previewRunId = previewRunResponse.data;
      previewRun = await fetchResult(firmId, previewRunId, templateType);
    }

    if (!previewOnly) {
      const testRunResponse = await sfApi.testing.createTestRun(firmId, testParams, templateType);
